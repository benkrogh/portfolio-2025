...
const MusicPlayer = () => {
  ...
  const animate = useCallback(() => {
    ...
    const draw = () => {
      ...
      const average = audioAnalyser.dataArray.reduce((acc, val) => acc + val, 0) / audioAnalyser.dataArray.length;
      ...
      gradient.addColorStop(0, `hsla(12, 85%, ${55 + normalizedIntensity * 30}%, ${0.5 + normalizedIntensity * 0.4})`);
      gradient.addColorStop(0.5, `hsla(${12 + hueShift}, 75%, ${50 + normalizedIntensity * 25}%, ${0.4 + normalizedIntensity * 0.3})`);
      gradient.addColorStop(1, `hsla(20, 65%, ${45 + normalizedIntensity * 20}%, ${0.3 + normalizedIntensity * 0.4})`);
            ...
            <div className="w-full flex flex-col sm:flex-row items-center gap-4 sm:gap-0 sm:justify-between">
              ...
              <div className="flex items-center gap-8 sm:w-1/2 justify-center">
                <button className="w-10 h-10 group relative flex items-center justify-center">
                  <div className={`absolute inset-0 bg-[#D6D2CB] rounded-[10px] transition-all duration-300 scale-90 opacity-0 group-hover:opacity-100 group-hover:scale-100 ${isPlaying ? 'group-hover:opacity-60' : ''}`} />
